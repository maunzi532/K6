...
public record AttackInfo4(XCharacter initiator, EquipableItem4 initiatorItem,
                          XCharacter target, EquipableItem4 targetItem, int distance)
{
	public static List<AttackInfo4> attackOptions(XCharacter initiator, XCharacter target, TileType y1)
	{
		...
		return initiator.systemChar().possibleAttackItems(distance).stream()
				.map(e -> new AttackInfo4(initiator, e, target, defendItem, distance))
